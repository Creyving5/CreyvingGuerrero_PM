...
// combiancion de variables globales y locales, y parametros por valor y por referencia.
...
void main(void)
{
    ...
    a = 1;  //Se asiggna un valor a la variable local a.
...
void funcion1( int *b, int *c)
{
    ...
    d = 3;       // Nota que se hace referencia a la variable local.
    (*b)++;
    (*c) += 2;
    printf("\n%d %d %d %d", a, *b, *c, d);
}
...
int funcion2( int c, int *d)
{
    int b;
    a++;
    b = 7;       // Observa que se hace referencia a la variable global a.
    c += 3;       // Nota que se hace referencia a la variable local.
    (*d) += 2;
    printf("\n%d %d %d %d", a, b, c, *d);
    return (c);
}
