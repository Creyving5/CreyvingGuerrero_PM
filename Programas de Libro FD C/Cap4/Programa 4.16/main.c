...
//Temperaturas.
//El programa recibe como datos 24 números reales que representan las temperaturas en el exterior en un periodo de 24 horas. Calcula el promedio del dia y las temperaturas maxima y minima con la hora en la que se registraron.

void Acutem (float);
void Maxima (float, int);          //Variables de funciones
...
int HMAX;
int HMIN;
 //Variables globales. ACT se utiliza para acumular las temperaturas, por esa razon se inicializa en cero. MAX se utiliza para calcular la maxima; se inicializa en –50 para que el primer valor que se ingrese modifique su contenido. MIN se usa para calcular la minima; se inicializa con un valor muy alto para que el primer valor ingresado
 //modifique su contenido. HMAX y HMIN se utilizan para almacenar el horario en que se produjeron las temperaturas maxima y minima, respectivamente. */
...
void main(void)
{
    ...
    for (I = 1; I <= 24; I++)
    {
        printf("Ingresa la temperatira de la hora %d: ", I);
        scanf("%f", &TEM);
        Acutem (TEM);
