...
//La funcion Lectura se utiliza para leer un arreglo tridimensional de tipo entero FI filas, CO columnas y PR profundidad. Observa que al ser tridimensional se necesitan tres ciclos para recorrer el arreglo.
{
    int K, I, J;
    for(K=0; K<PR; K++)
        for(I=0; I<FI; I++)
...
//Esta funcion se utiliza para determinar el ano en el que ingreso el mayor numero de alumnos a la universidad. Observa que el primer ciclo externo corresponde a los anos. */
{
    ...
    for (K=0; K<PR; K++)
        //Por cada ano se suma el ingreso de los dos semestres de las ochos carreras.
    {
        SUM = 0;
        for (I=0; I<FI; I++)
            for (J=0; J<CO; J++)
