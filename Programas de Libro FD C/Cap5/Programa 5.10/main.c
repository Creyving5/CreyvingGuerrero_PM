...
void main(void)
{
    ...
    if(RES)
        //Si RES tiene un valor verdadero -diferente de 0-, se escribe la posisicion en la que se encontro el elemento.
...
//Si se encuentra, la funcion regresa la posicion correspondiente. En caso contrario regresa 0.
{
    int RES, I = 0, BAN = 0;
    while (I < T && (E >= A[I]) && !BAN)
        //Observa que se hace una nueva incorporacion.
        if(A[I] == E)
            BAN++;
    else
        I++;
    if (BAN)
        RES = I +1;
    //Se asigna I+1 dado que las posisciones en el arreglo comienzan desde cero.
    else
        RES = BAN;
    return(RES);
}
