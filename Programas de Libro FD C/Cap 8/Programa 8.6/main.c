...
void main(void)
{
    ...
    while (OPE)
    {
        switch (OPE)
        {
            ...
            //Se pasa el parametro por refernecia, porque se puede modificar el numero de elementos del arreglo en la funcion.
            break;
...
// Esta funcion se utiliza para manejar las venta a un cliente. Se ingresan productos y cantidades, el fin de datos esta dado por el cero. Ademas de obtener el total de las ventas, se actualiza el inventario.
{
    int CLA, CAN, I, RES;
    float TOT, PAR;
    printf("\nIngrese xlave del producto -0 para salir: ");
    scanf("%d", &CLA);
    TOT = 0.0;
    while (CLA)
    {
            ...
            //Se analiza si el stock es suficiente para satisfacer el pedido.
        {
            ...
            printf("\nIngrese la siguiente clave del prodyucto -0 para salir -:");
            scanf("%d", &CLA);
    }
    printf("\nTotal de la venta: %f", TOT);
}

void Reabastecimiento(producto A[I], int T)
//Esta funcion se utiliza para rebastecer al Inventario.
{
    int CLA, CAN, I;
...
{
    int CLA, I, J;
    printf("\nIngrese clave del productto -0 para salir-: ");
    scanf("%d", &CLA);
    while ((*T <30) && (CLA))
    {
        I = 0;
        while ((I < *T) && (A[I].clave <CLA))
            ...
            I++;
        if(I == *T)    //Se inserta el elemento en la ultima psosicion.
        {
            A[I].clave = CLA;
            printf("\tNombre:");
            fflush(stdin);
        ...
        {
            for (J=*T; J>I; J--)
                //Se inserta el nuevoo producto en el arreglo. Se mueven una posicion a la derecha los elementos del arreglo que tengan una clave del producto  mayor a la ingresada.
               A[J = A[J - 1];
            A[I].clave = CLA;
            printf("\tNombre: ");
            fflush(stdin);
