...
//El programa calcula el producto de dos vectores y almacena el resultado en otro arreglo undimensional.
...
Lectura(VE1, MAX);
// Se llama a la funcion Lectura. Observa que el paso del arreglo a la funcion es por referencia. Sólo se debe incluir el nombre del arreglo.
Lectura(VE2, MAX);
...
Imprime(VE3, MAX);
 ...
 void Lectura(int VEC[], int T)
 //La funcion Lectura se utiliza para leer un arreglo unidimensional de T elemenmtos de tipo entero.
{
     int I;
     printf("\n");
     for (I=0; I<T; I++)
